...
int main(int argc, const char *argv[])
{
  ...
  cout << "programme Test du Binding Fortran/C" << endl;
  ...
  if (argc > 1)
  {
    for (int i = 0; i < argc; i++)
      cout << "i=" << i << " argv= " << argv[i] << " strlen(argv)= " << strlen(argv[i]) << endl;
  }
  ...
  int length = strlen(myMesh.mesh_name);
  cout << "length=" << length << endl;

  myMesh.mesh_name[length - 1] = '\0'; // end of the string
  */
  ...
  */
  ...
  cout << "length=" << length << endl;
  myMesh.mesh_name = (char *)malloc(sizeof(char) * (length + 1));
  strcpy(myMesh.mesh_name, argv[1]);
  myMesh.mesh_name[length + 1] = '\0'; // end of the string
